...
async function applyMigration() {
  ...
  const migrationPath = join(__dirname, '..', 'migrations', '0005_restore_data.sql');
  const sqlContent = readFileSync(migrationPath, 'utf-8');

  console.log('Applying migration 0005_restore_data.sql...');
  ...
  try {
    ...
    const templatesResult = await pool.query('SELECT COUNT(*) FROM specification_templates');
    const optionsResult = await pool.query('SELECT COUNT(*) FROM specification_options');
    
    console.log(`Categories: ${categoriesResult.rows[0].count}`);
    console.log(`Specification Templates: ${templatesResult.rows[0].count}`);
    console.log(`Specification Options: ${optionsResult.rows[0].count}`);
  } catch (error) {
